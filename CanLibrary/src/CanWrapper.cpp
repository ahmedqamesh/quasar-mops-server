...
bool CanWrapper::writeCanMessage(int cobid, int msg[], int dlc, bool extended,
	...
	auto result = CanWrapper::canMsgQueue(int(frame.can_id), vmsg, int(frame.can_dlc));
/*
	...
	if (!retval){
		...
		if (gotMessage) {
			...
			if (dlc == 8
				...
				std::cout << "]"<<"\n";
				*/
				CanWrapper::sdoData = (((int32_t)ret.at(nDatabytes + 3)) << 24) | (((int32_t)ret.at(nDatabytes + 2)) << 16) | (((int32_t)ret.at(nDatabytes + 1)) << 8) | ((int32_t)ret.at(nDatabytes + 0));
				output =true;
			}else {
				messageValid =false;
